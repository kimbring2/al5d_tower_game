...
bool ZedCamera::initializeZedCamera() {
  ...
  params.depth_mode = sl::DEPTH_MODE::DEPTH_MODE_ULTRA;
  ...
  zed_run_params_.enable_depth = true;
  zed_run_params_.enable_point_cloud = true;
...
void ZedCamera::retriveImages() {
  ...
  zed_->retrieveImage(depth_image_, sl::VIEW_DEPTH);
}
...
void ZedCamera::publishRgbData(int64_t acq_time, const sl::CalibrationParameters& camera_params) {
  ...
  Image3ub buffer_depth(zed_resolution_.height, zed_resolution_.width);
  ConvertBgraToRgb(BgraZedImageView(depth_image_), buffer_depth);
  show("depth_thumbnail", [&](sight::Sop& sop) { sop.add(buffer_depth); });
  ToProto(std::move(buffer_depth), d_camera.getImage(), tx_depth_camera().buffers());

  tx_left_camera_rgb().publish(acq_time);
  tx_right_camera_rgb().publish(acq_time);
  tx_depth_camera().publish(acq_time);
}
