...
void ZedCamera::retriveImages() {
  ...
  zed_->retrieveImage(depth_image_, sl::VIEW::DEPTH);
}
...
void ZedCamera::publishRgbData(int64_t acq_time) {
  ...
  show("depth_thumbnail", 
        [&](sight::Sop& sop)  { sop.add(Reduce<kSightReduceSize>(buffer_depth)); });
  ToProto(std::move(buffer_depth), tx_depth_camera().initProto(),
          tx_depth_camera().buffers());
