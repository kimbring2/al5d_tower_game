...
NLOHMANN_JSON_SERIALIZE_ENUM(RESOLUTION, {
    {RESOLUTION::HD2K, "2208x1242"},
    {RESOLUTION::HD1080, "1920x1080"},
    {RESOLUTION::HD720, "1280x720"},
    {RESOLUTION::VGA, "672x376"},
    {RESOLUTION::LAST, nullptr},
});
...
class ZedCamera : public alice::Codelet {
 public:
  ...
  // Left color image
  ISAAC_PROTO_TX(ImageProto, left_camera_rgb);
  // Right color image
  ISAAC_PROTO_TX(ImageProto, right_camera_rgb);
  // Left gray image
  ISAAC_PROTO_TX(ImageProto, left_camera_gray);
  // Right gray image
  ISAAC_PROTO_TX(ImageProto, right_camera_gray);
  // Camera pair extrinsics (right-to-left)
  ISAAC_PROTO_TX(Pose3dProto, extrinsics);
  // Intrinsics including pinhole and distortion parameters for the left camera
  ISAAC_PROTO_TX(CameraIntrinsicsProto, left_intrinsics);
  // Intrinsics including pinhole and distortion parameters for the right camera
  ISAAC_PROTO_TX(CameraIntrinsicsProto, right_intrinsics);
  // Intrinsics including pinhole and distortion parameters for the depth camera
  //ISAAC_PROTO_TX(CameraIntrinsicsProto, depth_intrinsics);

  ISAAC_PROTO_TX(ImageProto, depth_camera);
  ...
  ISAAC_PARAM(sl::RESOLUTION, resolution, sl::RESOLUTION::VGA);
  ...
  // The parameter value is ignored if :code:`auto_exposure` is enabled.
  ...
  // The gamma control. Valid values are between 1 and 9.
  // The special value -1 commands the camera ISP to auto-tune the parameter accordingly to the
  // lighting conditions.
  ISAAC_PARAM(int, gamma, sl::VIDEO_SETTINGS_VALUE_AUTO);
  // The hue control. Valid values are between 0 and 11.
  ISAAC_PARAM(int, hue, 0);
  // The saturation control. Valid values are between 0 and 8.
  ISAAC_PARAM(int, saturation, 4);
  // The digital sharpening control. Valid values are between 0 and 8.
  ISAAC_PARAM(int, sharpness, 4);
 ...
 private:
  ...
  void publishGrayData(int64_t acq_time);
  // Publish the RGB stereo image data
  void publishRgbData(int64_t acq_time);
  // Publish ZED camera intrinsics and extrinsics
  void publishCameraData(int64_t acq_time);

  // ZED camera state
  std::unique_ptr<sl::Camera> zed_;
  // Can't brace-initialize the structure as it has deprecated fields.
  sl::CameraInformation zed_info_;
  sl::CameraConfiguration zed_configuration_ = {};
